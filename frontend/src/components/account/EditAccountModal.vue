<template>
  ...
  >
    ...
    >
      ...
      <div
        v-if="((account?.platform === 'anthropic' && account?.type === 'apikey') || account?.platform === 'kiro') && webSearchGlobalEnabled"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between">
          <div>
      ...
      <!--
        Kiro: per-account device fingerprint (machine_id). Embedded into the
        User-Agent the gateway sends upstream (KiroIDE-<ver>-<machineId>),
        so changing it rotates the AWS-observed device for this account.
        Leave blank to fall back to the deterministic sha256 hash — stable
        but recognisable; for production paste the real GUID exported from
      ...
      >
        <!--
          Kiro Profile ARN — required by the upstream Kiro /generateAssistantResponse
          endpoint. Historically we treated it as optional for Builder ID
          accounts because older AWS builds would accept a missing field, but
          production now returns HTTP 400 "profileArn is required" when the
          payload omits it. We therefore expose a first-class editor so
          operators can correct accounts imported without it (e.g. captured
          from the CBOR RPC path where the field was NULL on the response).
        -->
        <div>
          <label class="input-label">
            {{ t('admin.accounts.kiro.profileArnLabel', 'Profile ARN（必填）') }}
          </label>
          <input
          ...
          <p class="mt-1 text-xs text-amber-600 dark:text-amber-400">
            {{ t('admin.accounts.kiro.profileArnHint', 'Kiro 上游 /generateAssistantResponse 要求此字段；请从 Kiro-account-manager 导出的账号 JSON 中复制 profileArn 字段粘贴。缺失会导致 400 profileArn is required。') }}
          </p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            type="text"
            class="input font-mono text-xs"
            placeholder="64-char hex, e.g. f3a1b2...   (paste from Kiro-account-manager)"
          />
          <p class="mt-1 text-xs text-amber-600 dark:text-amber-400">
            {{ t('admin.accounts.kiro.machineIdHint', '从 Kiro-account-manager 导出的账号 JSON 复制 machineId 字段；留空则使用账号 ID 的 sha256 哈希作为兜底（AWS 风控可识别为聚合代理）。') }}
          </p>
        </div>
      </div>
...
// kiroMachineId mirrors credentials.machine_id for Kiro accounts. The value
// is rendered into the User-Agent the gateway sends upstream, so changing
// it rotates the AWS-observed device fingerprint for this account.
// Operators paste a 64-char hex GUID harvested from Kiro-account-manager.
const kiroMachineId = ref('')
// kiroProfileArn mirrors credentials.profile_arn. The upstream Kiro
// /generateAssistantResponse endpoint treats this as mandatory on current
// AWS builds — missing it yields HTTP 400 "profileArn is required". We
// therefore surface it in the edit dialog so operators can backfill
// accounts whose initial import (CBOR RPC or legacy OIDC exchange)
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  // Load the per-account Kiro device fingerprint so the operator can
  // verify / rotate it from the edit dialog. Stored under credentials,
  // not extra, because the gateway hot path reads it via GetCredential.
  if (newAccount.platform === 'kiro') {
    const kiroCreds = (newAccount.credentials as Record<string, unknown> | undefined)
    kiroMachineId.value = (kiroCreds?.machine_id as string) || ''
    // Guard against legacy rows where operators pasted the raw JSON fragment
    // `"profileArn": null` straight into the Profile ARN input — that string
    // starts with a quote and is clearly not an ARN. Present an empty field
    // instead so the operator is forced to re-paste the real value.
    const rawProfileArn = (kiroCreds?.profile_arn as string) || ''
    kiroProfileArn.value = rawProfileArn.startsWith('"') ? '' : rawProfileArn
    // Web Search Emulation 同样适用于 Kiro 账号
    const wsVal = extra?.web_search_emulation
    if (wsVal === 'enabled' || wsVal === 'disabled') {
      webSearchEmulationMode.value = wsVal
    } else if (wsVal === true) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Kiro: persist profile_arn + machine_id into credentials and
    // web_search_emulation into extra. All three pieces of state are
    // independent of OAuth/Setup-Token branching above, so we merge into
    // whichever credentials/extra map the OAuth branch already built
    // (falling back to the original account payload when nothing else
    // touched them this round).
    if (props.account.platform === 'kiro') {
      const currentCredentials = (updatePayload.credentials as Record<string, unknown>) ||
        (props.account.credentials as Record<string, unknown>) || {}
      const newCredentials: Record<string, unknown> = { ...currentCredentials }

      // profile_arn is effectively mandatory on the live Kiro backend —
      // missing it triggers HTTP 400. We still allow clearing so operators
      // can recover from a corrupt paste, but we never persist a string
      // that clearly isn't an ARN (e.g. a stray JSON fragment like
      // `"profileArn": null`) because it would just reproduce the bug.
      const trimmedProfileArn = kiroProfileArn.value.trim()
      if (trimmedProfileArn && !trimmedProfileArn.startsWith('"')) {
        newCredentials.profile_arn = trimmedProfileArn
      } else if (!trimmedProfileArn) {
        delete newCredentials.profile_arn
      }

      const trimmed = kiroMachineId.value.trim()
      if (trimmed) {
        newCredentials.machine_id = trimmed
      } else {
        // Empty string clears the explicit binding so the gateway falls
        // back to the sha256 deterministic value. Use delete (not = '')
        // so the JSON omits the key entirely.
