export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      kiro: {
        profileArnLabel: 'Profile ARN (required)',
        profileArnHint:
          'The upstream Kiro /generateAssistantResponse endpoint requires this field — a missing value yields HTTP 400 "profileArn is required". Paste the profileArn from the account JSON exported by Kiro-account-manager, e.g. arn:aws:codewhisperer:us-east-1:xxx:profile/xxx.',
        machineIdLabel: 'Machine ID (Device Fingerprint)',
        machineIdHint:
          'Paste the per-account machineId (64-char hex) exported from Kiro-account-manager. Leave blank to fall back to a sha256 hash derived from the account ID — stable but AWS risk control can flag it as an aggregated proxy. Always fill the real value in production.',
      },
      anthropic: {
