export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      kiro: {
        profileArnLabel: 'Profile ARN（必填）',
        profileArnHint:
          'Kiro 上游 /generateAssistantResponse 要求此字段，缺失会返回 400 profileArn is required。请从 Kiro-account-manager 导出的账号 JSON 中复制 profileArn 字段粘贴，格式 arn:aws:codewhisperer:us-east-1:xxx:profile/xxx。',
        machineIdLabel: 'Machine ID（设备指纹）',
        machineIdHint:
          '从 Kiro-account-manager 导出的账号 JSON 中复制 machineId 字段（64 位十六进制）。留空时网关会用账号 ID 派生 sha256 兜底——稳定但 AWS 风控可识别为聚合代理，强烈建议填入真实值。',
      },
