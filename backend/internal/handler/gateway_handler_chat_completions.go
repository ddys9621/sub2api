...
func (h *GatewayHandler) ChatCompletions(c *gin.Context) {
	...
	for {
		...
		if account.Platform == service.PlatformKiro {
			result, err = h.kiroGatewayService.ForwardAsChatCompletions(c.Request.Context(), c, account, forwardBody, parsedReq)
		} else {
...
func (h *GatewayHandler) handleCCFailoverExhausted(c *gin.Context, lastErr *service.UpstreamFailoverError, streamStarted bool) {
	...
	if msg, ok := kiroInvalidModelErrorMessage(lastErr); ok {
		h.chatCompletionsErrorResponse(c, http.StatusBadRequest, "invalid_request_error", msg)
		return
	}
