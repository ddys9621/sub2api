...
type BatchUpdateCredentialsRequest struct {
	...
	// Field is the credentials key to overwrite. Kept as an explicit
	// allow-list so the admin endpoint cannot be used to clobber tokens
	// in bulk. `machine_id` is included so operators can pin the same
	// fingerprint across a group of Kiro accounts in one click (e.g.
	// when re-binding all accounts in a residential-IP pool to one
	// physical machine harvested from Kiro-account-manager).
	Field string `json:"field" binding:"required,oneof=account_uuid org_uuid intercept_warmup_requests machine_id profile_arn"`
...
func (h *AccountHandler) GetAvailableModels(c *gin.Context) {
	...
	// Handle Kiro accounts before the generic OAuth branch so per-account
	// model_mapping stays visible in the test-model picker.
	if account.Platform == service.PlatformKiro {
		mapping := account.GetModelMapping()
		if len(mapping) == 0 {
			if h.accountTestService != nil {
				if models, modelErr := h.accountTestService.GetKiroAvailableModels(c.Request.Context(), account); modelErr == nil && len(models) > 0 {
					response.Success(c, models)
					return
				} else if modelErr != nil {
					slog.Debug("kiro available models probe failed; using built-in model list",
						"account_id", account.ID, "error", modelErr)
				}
			}
		...
		var models []kiropkg.Model
		for requestedModel := range mapping {
			models = append(models, kiropkg.ModelFromID(requestedModel, ""))
		}
		response.Success(c, models)
		return
	}
