	...
	for {
		...
		for {
			...
			if account.Platform == service.PlatformAntigravity && account.Type != service.AccountTypeAPIKey {
				...
				result, err = h.kiroGatewayService.Forward(requestCtx, c, account, parsedReq)
			} else {
...
func (h *GatewayHandler) handleFailoverExhausted(c *gin.Context, failoverErr *service.UpstreamFailoverError, platform string, streamStarted bool) {
	...
	if platform == service.PlatformKiro {
		if msg, ok := kiroInvalidModelErrorMessage(failoverErr); ok {
...
func kiroInvalidModelErrorMessage(failoverErr *service.UpstreamFailoverError) (string, bool) {
	if failoverErr == nil || failoverErr.StatusCode != http.StatusBadRequest || !kiro.IsInvalidModelError(failoverErr.ResponseBody) {
		return "", false
	}
	msg := service.ExtractUpstreamErrorMessage(failoverErr.ResponseBody)
	if strings.TrimSpace(msg) == "" {
		msg = "Invalid model. Please select a different model to continue."
	}
	return msg, true
}
...
func (h *GatewayHandler) CountTokens(c *gin.Context) {
	...
	// Kiro does not support Anthropic count_tokens. Return before account
	// selection so this helper endpoint cannot trip OAuth 401 cooldowns.
	if apiKey.Group != nil && apiKey.Group.Platform == service.PlatformKiro {
		h.errorResponse(c, http.StatusNotFound, "not_found_error", "count_tokens endpoint is not supported for Kiro")
		return
	}
