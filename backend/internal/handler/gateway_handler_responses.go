...
func (h *GatewayHandler) Responses(c *gin.Context) {
	...
	for {
		...
		if account.Platform == service.PlatformKiro {
			result, err = h.kiroGatewayService.ForwardAsResponses(c.Request.Context(), c, account, forwardBody, parsedReq)
		} else {
...
func (h *GatewayHandler) handleResponsesFailoverExhausted(c *gin.Context, lastErr *service.UpstreamFailoverError, streamStarted bool) {
	...
	if msg, ok := kiroInvalidModelErrorMessage(lastErr); ok {
		h.responsesErrorResponse(c, http.StatusBadRequest, "invalid_request_error", msg)
		return
	}
