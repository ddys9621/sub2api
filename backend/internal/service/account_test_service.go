...
// GetKiroAvailableModels returns live Kiro model IDs for the account.
func (s *AccountTestService) GetKiroAvailableModels(ctx context.Context, account *Account) ([]kiro.Model, error) {
	if s == nil || s.kiroGatewayService == nil {
		return nil, errors.New("kiro gateway service not configured")
	}
	return s.kiroGatewayService.ListAvailableModels(ctx, account)
}
...
func (s *AccountTestService) testKiroAccountConnection(c *gin.Context, account *Account, modelID string, prompt string) error {
	...
	testModelID := modelID
	if testModelID == "" {
		testModelID = kiro.DefaultTestModel
	}
	...
	effectiveModelID := kiro.ResolveModel(testModelID, mapping)
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		if resp.StatusCode == http.StatusBadRequest && kiro.IsInvalidModelError(raw) {
			fallbackModel := kiro.FallbackModelForInvalidModel(testModelID, effectiveModelID)
			if fallbackModel != "" && fallbackModel != effectiveModelID {
				fallbackMapping := cloneKiroModelMapping(mapping)
				fallbackMapping[testModelID] = fallbackModel
				fallbackPayload, berr := kiro.BuildKiroPayload(anthropicReq, kiro.BuildOptions{
					ProfileArn:   profileArn,
					ModelMapping: fallbackMapping,
				})
				if berr != nil {
					return s.sendErrorAndEnd(c, fmt.Sprintf("Failed to build Kiro fallback payload: %s", berr.Error()))
				}
				fallbackBody, merr := json.Marshal(fallbackPayload)
				if merr != nil {
					return s.sendErrorAndEnd(c, fmt.Sprintf("Failed to marshal Kiro fallback payload: %s", merr.Error()))
				}
				_ = resp.Body.Close()
				req, err = http.NewRequestWithContext(
					forwardCtx,
					http.MethodPost,
					kiroUpstreamEndpointForAccount(account, profileArn),
					bytes.NewReader(fallbackBody),
				)
				if err != nil {
					return s.sendErrorAndEnd(c, fmt.Sprintf("Failed to build Kiro fallback request: %s", err.Error()))
				}
				req.Header = kiroUpstreamHeaders(token, account)
				resp, err = client.Do(req)
				if err != nil {
					return s.sendErrorAndEnd(c, fmt.Sprintf("Kiro fallback request failed: %s", err.Error()))
				}
				defer resp.Body.Close()
				effectiveModelID = fallbackModel
				if resp.StatusCode >= 200 && resp.StatusCode < 300 {
					goto kiroTestResponseOK
				}
				raw, _ = io.ReadAll(io.LimitReader(resp.Body, 16*1024))
			}
		}
...
kiroTestResponseOK:
	...
	s.sendEvent(c, TestEvent{Type: "model_info", Model: effectiveModelID})
