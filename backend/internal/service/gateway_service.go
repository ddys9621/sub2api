...
func (s *GatewayService) calculateRecordUsageCost(
	...
	if result.KiroMeteringCredit > 0 {
		// 渠道定价显式配置时优先使用渠道定价；否则 Kiro 平台永远会被
		// metering credit 抢先返回，管理员配置的 Kiro 模型价格不会生效。
...
func (s *GatewayService) ForwardCountTokens(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) error {
	...
	// Kiro/CodeWhisperer does not expose Anthropic's count_tokens endpoint.
	// Calling Anthropic with a Kiro bearer token returns 401 and incorrectly
	// puts the OAuth account into a temporary unschedulable state.
